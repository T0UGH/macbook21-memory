...
原因：
- thread 是一等公民
- 协作历史本质上是 thread 历史，不是单邮件孤岛
- obligation 与闭合条件也更适合挂在线程上
...
- 没有 open obligation
- 最新邮件类型落在可自然收尾的类型上，例如：
  - `inform`
  - `report`

这更符合人类邮件系统的自然结束方式，而不是工单式手动关闭。
...
23. 基于 thread event log 的生命周期建模
24. 未回复 request 的可观察性
25. 一个 CLI 包 + 一个 skill 的交付形态
26. 基于少量规则 + agent 自主判断的处理选择机制
